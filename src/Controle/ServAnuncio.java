...
public class ServAnuncio extends HttpServlet {
	...
	//Constantes com os endere�os das paginas de cada etapa
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(step==this.STEP_0)
		{
			...
			//Sess�o em modo de cadastro
			sessao.setAttribute("edit","false");

			//Encaminha para p�gina de escolha o da plano e do tipo de ve�culo	
		...
		//ESCOLHA DE FOTOS
			...
			//Recupera o veiculo em sessao para recuepra��o de id
		...
		else if(this.step==this.EDIT_ANUNCIO)
		{
			...
			//Recupera o tipo de ve�culo
			String tipo =  new VeiculoDAO().getTipo(id_veiculo);
			
			//Verifica se o anuncio pertence ao us�rio em sess�o
			
			
			//Joga o anuncio em sess�o para edi��o dos dados
			...
			response.getWriter().write("P�gina de destino n�o definida");	
		...
		//CADASTRO DE UM ANÚNCIO NO BANCO
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(step==this.STEP_1)
		{
			...
			//Atribui��o de valores de acordo com o plano escolhido

			//Recupera o buffer de passos

			//Atribui��o do veiculo ao anuncio
			...
			//Emcaimnha para a proxima etapa (Cadastro de informa��es basicas)
			if(tipoVeiculo==1)
		    this.setCadInfo(this.PAGE_CAD_INFO_CARRO);	
			else if(tipoVeiculo==2)
		    this.setCadInfo(this.PAGE_CAD_INFO_MOTO);	
		...
		else if(step==this.STEP_2)
		{
			...
			if(tipo==1 || an.getVeiculo().getClass()==Carro.class)
			{
                 ...
                 //Se os dados j� estiverem em sessao
			...
			System.out.println("O tipo de ve�culo � "+tipo);
			...
			//Joga o código do veículo na sessão
			sessao.setAttribute("cod_veiculo",an.getVeiculo().getId());

			//Vai para p�gina de escolha de fotos 
			if(mostra_step==1)	 
		...
		//Ao final encaminha para pagina de escolha de fotos
			...
			//RECUPERA O TIPO DE VE�CULO QUE EST� SENDO CADASTRADO
			int tipo = (Integer) sessao.getAttribute("tipo_veiculo");
			...
			//Recupera o anuncio em sess�o
			Anuncio an = (Anuncio) sessao.getAttribute("anuncio");

			//Atualiza o carro em anuncio
			an.setVeiculo(carro);

			//Joga novamente o anuncio em sess�o
			sessao.setAttribute("anuncio",an);
			...
			else if(tipo==2)
			{
				...
				Moto moto = (Moto) sessao.getAttribute("moto");
				 ...
				 System.out.println("Moto na sess�o: "+moto);
	...
	//Configura a sess�o de an�ncio para edi��o
	...
	//Define na sess�o os parametros iniciais para anunciar um ve�culo
	private void resetSessaoAnuncio(HttpSession sessao){
		...
		//Define as paginas que j� foram preenchidas
		{
		...
		//Define a data de cadastro do an�ncio
		...
		//Localiza��o generica para o anuncio
	...
	private StringBuffer  getStepsHtml(HttpSession sessao,int atual_page){
		...
		case 1:
			step1="<span>1. Informa��es b�sicas</span>";
			step2="<a href=\""+this.getLinkStep(2,sessao)+"\"><span>2. Contato</span></a>";
			step3="<a href=\""+this.getLinkStep(3,sessao)+"\"><span>3. Imagens</span></a>";
			step4="<a href=\""+this.getLinkStep(4,sessao)+"\"><span>4. Pr�-Visualizar</span></a>";
			step5="<a href=\""+this.getLinkStep(5,sessao)+"\"><span>5. Conclu�do com sucesso!</span></a>";
			break;


		case 2:
			step2="<span>2. Contato</span>";
			step1="<a href=\""+this.getLinkStep(1,sessao)+"\"><span>1. Informa��es b�sicas</span></a>";
			step3="<a href=\""+this.getLinkStep(3,sessao)+"\"><span>3. Imagens</span></a>";
			step4="<a href=\""+this.getLinkStep(4,sessao)+"\"><span>4. Pr�-Visualizar</span></a>";
			step5="<a href=\""+this.getLinkStep(5,sessao)+"\"><span>5. Conclu�do com sucesso!</span></a>";
			break;
		...
		case 4:
			step4="<span>4. Pr�-Visualizar</span>";
			step2="<a href=\""+this.getLinkStep(2,sessao)+"\"><span>2. Contato</span></a>";
			step1="<a href=\""+this.getLinkStep(1,sessao)+"\"><span>1. Informa��es b�sicas</span></a>";
			step3="<a href=\""+this.getLinkStep(3,sessao)+"\"><span>3. Imagens</span></a>";
			step5="<a href=\""+this.getLinkStep(5,sessao)+"\"><span>5. Conclu�do com sucesso!</span></a>";    
		...
		case 5:
			step5="<span>5. Conclu�do com sucesso!</span>";
			step2="<a href=\""+this.getLinkStep(2,sessao)+"\"><span>2. Contato</span></a>";
			step1="<a href=\""+this.getLinkStep(1,sessao)+"\"><span>1. Informa��es b�sicas</span></a>";
			step3="<a href=\""+this.getLinkStep(3,sessao)+"\"><span>3. Imagens</span></a>";
			step4="<a href=\""+this.getLinkStep(4,sessao)+"\"><span>4. Pr�-Visualizar</span></a>";    
	...
	//RECUPERA O ESTADO DE PREENCHIMENTO DE UMA P�GINA
	...
	private String getLinkStep(int step,HttpSession sessao){
		...
		//Verifica se a pagina j� foi preenchida
	...
	//Tranforma um vetor de String e um vetor de Integer (Para obten��o dos acess�rios do ve�culo)
	...
	private Carro compCarro(Carro carro,HttpServletRequest request){
		...
		System.out.println("Vers�o: "+carro.getVersao());
	...
	//COMPLETA OS DADOS DA MOTO A PARTIR DO REQUEST
		...
		public void finalizarAnuncio(HttpSession sessao,HttpServletRequest request,HttpServletResponse response) throws IOException{
			...
			if(user!=null)	
			{
			 //Recupera o anuncio da sess�o	
			Anuncio an = (Anuncio) sessao.getAttribute("anuncio");	
			
			
			
			
			//Joga o id do veiculo na sess�o
			sessao.setAttribute("cod_veiculo",an.getVeiculo().getId());
			
			
		
			
			//Seta o usu�rio respons�vel pelo an�ncio
			...
			//----SE O PLANO FOR GR�TIS----	
			//*OS DADOS DO VE�CULOS S�O SALVOS EM ESPERA PARA ANALISE DOS AGENTES DO SISTEMA
			//*REDIRECIONA PARA A P�GINA DE AN�NCIO CADASTRADO COM SUCESSO	
			...
			//----SE O PLANO FOR PAGO----		
			//*REDIRECIONA PARA A P�GINA DE PAGAMENTO E CONFIRMA��O DE PAGAMENTO
			//REDIRECIONA PARA A P�GINA DE AN�NCIO CADASTRADO MAS AGUARDANDO PAGAMENTO	
			else {
			...
			else 
			{
				...
				Debug.gerar("Controle","ServAnuncio","SOLI=5(GET)","usuario na sesss�o = null");
			}
